...
std::vector<Day> Schedule::get_schedule_by_student(int s_id) const {
    ...
    for (auto c_id : s_courses) {
        query = "SELECT id FROM lesson WHERE course_id='" + std::to_string(c_id) + "';";
        if (!postgres.query(query, &result)) {
            throw std::exception();
        }
    ...
    for (auto l_id : lesson_ids) {
        query = "SELECT * FROM day WHERE lesson_id='" + std::to_string(l_id) + "';";
        if (!postgres.query(query, &result)) {
            throw std::exception();
        }
        ...
        Day curr_day(d_id, d_weekday, d_date, l_id);
...
std::vector<Day> Schedule::get_schedule_by_course(int c_id) const {
    std::string query = "SELECT id FROM lesson WHERE course_id='" + std::to_string(c_id) + "';";
    PGresult *result = nullptr;
    if (!postgres.query(query, &result)) {
        throw std::exception();
    }
